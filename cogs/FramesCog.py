...
class FramesCog(commands.Cog):
    ...
    async def frame_not_found_error(self, ctx):
        await ctx.send(f"**{ctx.author.mention} this frame does not exist.**")

    async def frame_not_owned_error(self, ctx):
        await ctx.send(f"**{ctx.author.mention} you do not own this frame.**")
    ...
    async def remove_frame(self, ctx, card_id_tag):
        ...
        await ctx.send(f"**{ctx.author.mention} you have successfully removed the frame.**")
