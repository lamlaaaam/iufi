...
class GiftSCCog(commands.Cog):
    ...
    async def giftsc(self, ctx, rec: discord.Member=None, amt: int=0):
        if rec == None:
            await ctx.send(f'**{ctx.author.mention} pick a recipient for your gift.**', delete_after=1)
            return
        ...
        if amt <= 0 or amt > currency:
            await ctx.send(f'**{ctx.author.mention} invalid amount for gifting.**', delete_after=1)
            return

        await db_utils.update_user_currency(ctx.author.id, -amt)
        await db_utils.update_user_currency(rec.id, amt)
        await ctx.send(f'**{ctx.author.mention} has gifted {rec.mention} {amt} starcandies.**', delete_after=1)
